...
**Gates.** For a click or a fill, these checks come after the target gates (confidence, runner-up, repeat) and before any text request or dialog. For Enter, they come before the confidence check, so the click on the submit button that can take the place of a low Enter (section 5.1) also meets them:
