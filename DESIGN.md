...
`extractSpans()` collects names to mention (section 5.6), quoted text, email addresses, URLs, dates, numbers, phrases after value verbs, proper nouns, short clauses, and the whole task. Profile names and catalog aliases are excluded. A proper noun can have letters of any script: "Gödel’s" is one word (the old ASCII rule cut it to "G"), and a curly apostrophe belongs to the word. "About" is a value verb. The topic after it ("the Wikipedia article about Gödel’s incompleteness theorems") is a query for a search box or another exact-value field. A field that can take new text never offers it, so "a note about the launch" does not type "the launch" as the note. A topic that the task also gives as a value elsewhere ("type Rust") takes that source, and every field offers it. A topic that is a name (capitalized words: "Project Phoenix", "Alan Turing") stays a proper noun with the topic mark (`Span.topic`): a single-line field (a title, a subject, a search box) offers it, and a multiline field (a message, a note) does not. "About" in capitals is a page or link name, and "about 10 minutes" is an approximation: neither starts a topic. The vercel engine drops topic spans, so its value question offers the spans that it offered before. Spans have stable request identifiers and bounded text. Whitespace normalization and candidate limits are defined in `src/task.ts` and `src/types.ts`. A span whose whole text is a date also gets that date, and two dates of a range get the roles start and end (section 5.7).
...
In MCP runs, the assistant can add generated spans; in CLI and chat runs, the text model can (section 8). A generated span has the id `g<n>`, the source `generated`, and `secret: false`. Its `field` binds it to one field: the key `<doc>|<node>` (the document's `performance.timeOrigin` and the node identity at request time), the redacted field label, and the text request id. A step request offers only the generated spans of the current document, because a span of another document can never pass the binding check. When a new document loads during the wait for text, the spans of that request are removed, and the re-ask says that the text was dropped. Without the text model the CLI and chat never create generated spans.
...
- Do the steps of the goal in the order that the goal gives them. Open a result only after the requested search and every requested filter are applied (`ORDER_RULE`). On the travel fixture of browser-use/jev-ultrafast ("use the destination search and filters to find Design stays in Lisbon with Free cancellation, then open Casa Flora"), the first step opened Casa Flora with no search or filter in 9 of 9 runs. With this rule, TYPE_TEXT on the search box led (0.43-0.47 against 0.31-0.37), and 2 of 3 runs did the whole flow.
...
Direct-engine rules also cover applying every requested filter, opening a requested result, and scrolling until an extraction value is visible. The direct engines offer `GO_BACK` only when the tab has an earlier web page (`Page.canGoBack`). Every tab opens on about:blank, so a back from the start page left the site for a blank page: on Google Flights, a run went back 5 to 6 times after it picked "London" and stopped with `loop_detected`.

The goal is in the state and in the instructions of the operation and target questions. A test without the copy in the state fixed the travel fixture (TYPE_TEXT 0.62-0.71 against 0.24), but the bench then lost the command bar (Enter picked "Ask AI" in 5 of 6 runs) and the invite form (Role was clicked at 0.28 before the email in 2 of 3 runs). With the copy, 6 of 6 passed, so it stays. A page with a date field or a calendar day also gets the date rule (section 5.7). Exact prompt strings live in the engine's policy or questions module.
...
**Vars in plugin runs.** In a run with a text source, a value question shows a non-secret var as `{ var: <key>, value: <text> }`. The assistant chooses the key, and SKILL.md tells it to name the key after the field label. With the key, a vars-only question gave "message" 0.81-0.84 against 0.67-0.70 without it. Without a text source the CLI shows the text alone.
...
The step after a send first waits while a rendered control still shows the text (a "sending" state). It observes every `waitPollMs` (100 ms), at most `fastWaitMs` (1.5 s). The workflow builder shows "sending" for 250 ms, and a DONE on that page fell below its gate. The loop keeps each sent text with its label and step (`sentTexts()`) and marks the record of the typed text as sent (check 4 above). The step request then gets `sent_texts` (each text redacted and cut to 120 characters) and the DONE_SENT text (section 6.1). Without a text model the CLI and chat have no unsent text, so they have no send record, and their requests do not change.
...
**Early decisions.** After a click, a fill, or Enter, the observation comes early (`Page.observe` with `early`): after the first frames of the settle and one check of the tracker (section 7.1). The rest of the settle runs in the page. The next step request goes out on the early page, and the loop finishes the settle while Jev answers. The answer holds only when the settled page has the same fingerprint as the early one and the step request built on the settled page is the same as the one that went out: the focus, the option that Enter picks, and what the settled page taught the run (chip values, held text) are in the request too. Then the loop continues on the settled observation. Otherwise the loop drops the answer and asks again on the settled page. A request that fails while the settle runs fails the run as any failed request does when the settled page is the same; when the page changed, the loop drops it with its error and asks again. The late observation also updates what `execute()` read from the early one (`lateRefresh`): the page change and URL of the history entry, the chip fields that a click or Enter taught, whether the text of a fill stayed and a chip-field value that a late popup shows, the chips that it added, where the texts of an audit went, and the step that a submit click opened. The stall check waits for that update. A run that ends right after an early action (the step cap, the run timeout, a cancel) ends the settle before its result, so the audit, the sent texts, and the final URL read the settled page. A select never arms the causal settle, so it has no early observation. A back, a wait, and a date fill wait for the whole settle. A fill into a field whose popup has no ARIA link (a command menu, a mention list) gets its `enter_picks` only from the end of the settle, so the request on the early page differs and the loop asks again: such fills gain no time. A send that the early page recorded and the settled page reverses (the text is back in a control) is no send: its entry is watched again. A dropped request costs tokens and, when the second socket is open, no time: the transport sends the next request on it (section 8). On 10 bench cases (mcp, with approval), early decisions took 44.6 s against 50.3 s, with all 20 runs passing; on Google Flights, 6 or 7 of 10 early requests were dropped, because the page kept changing. `JEV_EARLY_DECISIONS=0` turns them off.

**Answer check.** Before any code reads a choice answer, `createOracle` checks its shape, as `validate_choice` of browser-use/jev-ultrafast does: the choice and each probability key are options of the question, each probability is a finite number from 0 to 1, the sum is 1 (0.02 tolerance), the confidence is a number from 0 to 1, and the choice has the highest probability. A bad answer sends the request one time more. A head that is bad again is dropped, so a missing head never runs an action. In 434 logged answers, none was bad; one live run then got a choice that did not have the highest probability.
...
`GO_BACK` needs an operation confidence of 0.50 (`GATES.goBack`): it leaves the page and the text in its fields. Every GO_BACK in the ultrafast comparison runs had 0.16-0.44, and each one was wrong. Below the gate, the loop asks again without offering GO_BACK in that step.
...
- With a mention intent, the operation and target questions add `MENTION_RULE`: add the person with the field's mention picker, confirm with Done or Add, a checked item is not added, typed text is not a mention, and send only when the field lists every requested mention. They leave out `ORDER_RULE` ("do the steps in the order that the goal gives them"): "Post the update and mention Ann" names the send first, and the mention rule was measured without the order rule.
...
| `needs_text` | MCP runs, and CLI and chat runs with the text model: no usable new text. No writer is attached, the writer declined, no text came in 300 s (30 s for the text model), the text failed its checks, or the run used its 3 text requests |
...
2. Read the field (`editScript`, step `read`): its text, its kind, and its shape (`FieldShape`). Focus must be on the field or on an element inside it, and the field must be visible. One exception: a field that opens a popup (a combobox, `aria-haspopup`, `aria-expanded`, or an input with a list) whose click moved focus to the text input of a popover. That input must be a visible, editable one-line text input or textarea of a combobox, searchbox, autocomplete, or popup. A person types there, so every step of the fill uses it, and the log names it (`EditStep.moved`). On Google Flights, "Where from?" moves focus to "Where else?", and the fill was refused 2-4 times in each run. A plain field whose click moves focus to another plain field is still refused.
...
- Poll chains stop. A timer more than `causalGenerations` (4) callbacks deep does not count. A callback that schedules itself again with the same or a longer delay does not count. A debounce that waits again for the rest of its time has a shorter delay, so it still counts. A callback that schedules a new closure of its own code with the same delay counts as the same callback: on Google Flights, a 500 ms timer armed a copy of itself two times after each input, and each settle took 1.0-1.4 s instead of 0.5-0.6 s. A bound function shows as native code, so it is never such a copy: a poll chain of bound callbacks counts until the generation cap. A wrapper that many callbacks share (for example lodash `_.delay`) with the same delay counts as a copy. An interval counts until its first run.
- An early observe (section 5.2) waits for the first frames, for the options of an editable combobox (up to 200 ms), and for one check that closes the input window. When work is still open, it reads the page and leaves the rest for the next observe (`Page.pending`). A new input first ends the rest of the settle: its work belongs to the input before. The debug log gives one line per settle: its time, its checks, why it ended, the timers, and the requests with the last one that ended.
...
Each session creates and owns a tab. In attach mode, create the tab in the background and retain the existing viewport. Closing an attached session closes its owned tabs and connection, not the user's browser. A new tab enables the Page domain and focus emulation only. Evaluations need no Runtime domain, and `Runtime.enable` sent every console message of the page to this process. A headless Chrome sends "HeadlessChrome" in its user agent, and sites that block bots check for it, so the tab of a headless Chrome gets its user agent with "Chrome" instead (`plainUserAgent`, `Emulation.setUserAgentOverride`). The override also sends the client hints of that Chrome (`userAgentMetadata`: the brand list by Chrome's own algorithm, the platform, the OS version as Chrome reports it (`sw_vers` on macOS, the kernel version on Linux, empty on Windows), the architecture, and the form factor). Without them Chrome stops the Sec-CH-UA headers and empties `navigator.userAgentData`, a mismatch that bot checks look for. The override covers the page, its requests, and its dedicated workers. A headed Chrome keeps its own.
...
One HTTP transport serves key validation, model requests, and optional warm-up calls. It keeps two sockets alive (`SOCKETS`) and limits warm-up time. The API answers the requests of one connection one at a time, also over HTTP/2 (two at once: 295 and 596 ms; on two connections: 360 and 353 ms). So a step request that an early decision drops (section 5.2) never delays the next one. A warm-up sends one GET per socket at the same time, and nothing when both sockets are open. A run warms up while Chrome starts when its plan sent no Jev request. After a plan request, it warms up only while the page still loads, so the second socket is open before the first step; on the current page, or when the page loaded before the plan answered, a warm GET would delay the first step request. Chat sends idle warm-up requests while waiting for input. The MCP server sends them every 45 s for 10 minutes after a run ends, only when a key is loaded and no run is active; it sends none before the first run ends. A running task owns its model traffic. Closing the transport aborts outstanding warm-up work so it cannot delay exit. With no request in flight, it also ends a connect that is still open, which only a warm-up can need.

**Text model.** The CLI and chat can use a small language model for new field text, as the text helper of browser-use/jev-ultrafast does (`src/writer.ts`). It is off unless `JEV_TEXT_MODEL` and `JEV_TEXT_API_KEY` are set; `JEV_TEXT_BASE_URL` names an OpenAI-compatible endpoint (default OpenRouter), and `JEV_TEXT_REASONING` can be `none` or `low`. It is the run's text source, so it gets the same text request as an MCP assistant and serves only the fields that `generate` offers (fields that pass `canWriteInto`): never a password, code, search, or other exact-value field. It asks for a JSON object with one key per field id. A value that `checkTexts` rejects asks one time more with the rule, never the text. No `f1` is a decline. The log names the model, the time, and the fields, never a value. Its text is assistant text: the unsent-text gate asks before a click or Enter sends it, and the terminal prompt shows it. The text request goes to that endpoint: the redacted task, the page URL and title, the labels and current values of the fields (up to 2,000 characters of text already in the field), the recent actions with their typed text, the texts sent in this run, and up to 6,000 characters of page text (the text request below). The MCP server never uses it.
...
`bin/jev-browser.js` loads the package-local `.env` when present and starts the TypeScript entry point. API access requires `TYPESAFE_API_KEY`. Chat can use its saved key. `TYPESAFE_DEFAULT_MODEL`, `JEV_CHROME_BIN`, `JEV_CHROMIUM_BIN`, `JEV_BROWSER_BIN`, `JEV_BROWSER_MAX_STEPS`, `JEV_EARLY_DECISIONS` (section 5.2), the text model variables (section 8), profile/session variables, and configuration overrides are resolved by their owning modules.

A confirmation prompt in the terminal (CLI and chat) lists each text that the action can send, with its field, before "Type y to allow" (`confirmTexts`): the assistant or text-model texts of the unsent-text gate and the message fields that a send, a submit, or Enter sends. In the ultrafast comparison, a person allowed a send and did not see that the field held only the second line of the message.
