...
// name of the form's default button, `submitDefault`, and `multiline`, for the Enter-to-click rule. A single-line
// text input also carries its chip facts, `token` (see TokenFacts in model.ts); they never reach Jev either.
...
export const SNAPSHOT_SCRIPT: string = String.raw`(() => {
  ...
  const REMOVE=/^(?:remove|delete|clear|deselect|unselect|dismiss)\b|^[x×✕✖⨯]$/i, SEND=/\b(?:send|post|reply|comment|submit)\b/i;
  const POPUP='[role="listbox"],[role="menu"],[role="dialog"],[role="tree"],[role="grid"],[data-radix-popper-content-wrapper]';
  ...
  cache.popupsOf=(e,box,all)=>{
    ...
    for (const p of all||document.querySelectorAll(POPUP)) {
  ...
  const actions=[], popups=[...document.querySelectorAll(POPUP)].filter(visible);
