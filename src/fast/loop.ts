...
import type { Action, Chrome, FastHistoryEntry, FillOptions, Observation, Page, UnsentText } from "./model.js";
...
import { actionKey, buildStep, buildValueStep, canPressEnter, canWriteInto, cutLines, cutText, mentionIntent, mentionMatches, readStep, readValue, top3 } from "./policy.js";
...
export class FastRunner {
  ...
  private async apply(d0: Decision, obs: Observation, meta: StepMeta, ctx: StepCtx): Promise<Applied> {
    ...
    if (d.operation === "PRESS_ENTER") {
      ...
      ctx.value = "Enter";
      // Enter outside a picker with checked items closes it, and the items are lost. Enter in the picker's own field adds them.
      ...
      const stray = this.strayChip(obs, focusForm, obs.focus?.node ?? null);
      if (stray !== null) {
        ctx.gate = stray;
        return { kind: "reask" };
      }
      ...
      const denied = await this.confirmAction(risk, `press Enter on "${label || "the focused element"}"`, ctx, opTop, obs.url, this.pruneUnsent(obs), this.sentFields(obs, focusForm, obs.focus?.node ?? null));
  ...
  private async targeted(d: Decision, op: TargetOp, obs: Observation, _meta: StepMeta, ctx: StepCtx, floor?: RiskClass): Promise<Applied> {
    ...
    // action re-asks one time and then runs.
    const staged = this.stagedPicker(obs);
    ...
    let fill: FillOptions | undefined;
    if (op === "TYPE_TEXT") {
      // A select-all fill would remove the mention chips that this run added. Ask before any text request.
      const early = this.fillMode(action, obs);
      if (early !== null && early !== "append") {
        ctx.gate = early.gate;
        this.stepBans.add(key);
        return { kind: "reask" };
      }
      ...
      const mode = this.fillMode(target, at);
      if (mode !== null && mode !== "append") {
        ctx.gate = mode.gate;
        this.stepBans.add(key);
        return { kind: "reask" };
      }
      if (mode === "append") { fill = { append: true }; gate += " append"; }
    ...
    const denied = await this.confirmAction(risk, desc, ctx, top3(t.probs), at.url, gated, sends ? this.sentFields(obs, action.form ?? null, null) : []);
    if (denied) return { kind: "record", rec: denied };

    const r = await this.execute(op, target, text, shown, at, ctx, kept, fill);
   ...
   * them. `sends`: the message fields that a send, a submit, or Enter sends. The dialog shows each one that holds mention
   * chips or a text that `gated` does not show, so the user sees what goes out also when no assistant wrote it.
   */
  private async confirmAction(risk: RiskClass, desc: string, ctx: StepCtx, top: Top, url: string, gated: Unsent[] = [], sends: SentField[] = []): Promise<StepRecord | null> {
   ...
   * history. `fill`: how a fill types (see `fillMode`).
   */
  private async execute(op: string, action: Action | null, text: string | null, shown: string | null, obs: Observation, ctx: StepCtx, kept: string | null = shown, fill?: FillOptions): Promise<Applied> {
    const page = this._page as Page;
    ctx.action = ACTION_OF[op] ?? "none";
    ...
    try {
      ...
      else if (control && fill) await page.act(control, obs, text ?? undefined, fill);
  ...
  /**
   * How a fill types into field `a`. The field's chips are all mentions that this run added and that the task asks for:
   * - with no other text in the field, the fill puts its text at the end with no select-all ("append"), so the chips stay;
   * - with other text, a select-all fill would remove the chips: a gate text, and the loop asks again.
   * Null for a plain fill: a field without chips, with other atoms, or with a chip that this run did not add or the task
   * does not ask for. A select-all then replaces the chips, as before.
   */
  private fillMode(a: Action, obs: Observation): "append" | { gate: string } | null {
    const chips = a.mentions ?? [];
    if (chips.length === 0 || (a.otherAtoms ?? 0) > 0 || !chips.every((m) => this.added(obs, m) && this.asked(m))) return null;
    if ((a.bareText ?? "").trim() === "") return "append";
