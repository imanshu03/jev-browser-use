...
describe("chip (token) fields", () => {
  ...
  function chipPage(s: ChipState): Observation {
    ...
    const actions = [
      ...
      ...(pop?.picks ?? []).map((label, i) => el(`e${6 + i}`, "click", label, "button", { node: 60 + i, form: 9 })),
    ];
  ...
  it("the popup settle reads until two reads agree and nothing loads, and stops after tokenSettleMs worth of reads", async () => {
    ...
    const t = chips([fill("textbox", "v_n"), fill("Title (Optional)", "v_title"), click(MEMBER), click(SUBMIT), finish],
      { multi: true, chips: ["bob@example.com"], popup: listed }, { vars: { ...VARS, n: "Ann Lee" } }, { popups: (n) => (n < 3 ? searching : listed) });
    const r = await t.runner.run();
    expect(r.outcome).toBe("done");
    // Three busy reads, then the list two times: no script Enter, a hint, and Jev clicks the member.
    expect(t.page.popups).toHaveLength(5);
    expect(ops(t)).not.toContain("commit e1");
    const changing = chips([fill("textbox", "v_b"), fill("Title (Optional)", "v_title"), fill("Title (Optional)", "v_title"), click(SUBMIT), finish],
      { multi: true, chips: ["x"] }, {}, { popups: (n) => ({ open: true, text: `result ${n}`, picks: [], busy: false }) });
    await changing.runner.run();
    expect(changing.page.popups.length).toBe(1 + Math.ceil(LIMITS.tokenSettleMs / LIMITS.tokenPollMs));
  });
